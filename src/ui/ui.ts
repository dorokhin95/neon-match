...
export class UI {
  ...
  /** Модалка-заглушка (fallback только для browser/dev, где нет рекламного
   *  SDK): после «просмотра» выдаёт заявленную награду. В production-сборках
   *  Telegram/Yandex не используется (требование площадок — без имитации рекламы). */
  adStubModal(
    rewardName: string,
    onGranted: () => void,
    onCancel: () => void,
